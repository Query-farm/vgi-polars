...
"""`required_filters` cost-safety enforcement and the `rff_rowid` ordinary-column behavior.

Covers `_source.py::_check_required_filters` and the ordinary-column behavior of
a `row_id`-flavored table (`rff_rowid`).
...
def test_scan_or_group_satisfied_by_either_column(catalog: vp.VgiCatalog) -> None:
    """A filter on just one column of an OR-group must still satisfy it.
...
def test_scan_or_group_unsatisfied_by_unrelated_column_raises(catalog: vp.VgiCatalog) -> None:
    """A predicate on an unrelated column must still fail the OR-group requirement.
...
def test_scan_struct_subfield_requirement_conservative_approximation(catalog: vp.VgiCatalog) -> None:
    """A predicate touching the top-level struct column satisfies every subfield group.
...
def test_scan_multi_requires_every_and_group(catalog: vp.VgiCatalog) -> None:
    """A predicate satisfying only one of two AND'd required-filter groups must still raise.
...
def test_rff_rowid_row_id_is_an_ordinary_column(catalog: vp.VgiCatalog) -> None:
    """`row_id` is a normal declared schema column, not a hidden virtual one.
